...
class RegionAdmin(admin.ModelAdmin):
    ...
    def climate_badge(self, obj):
        ...
        except ClimateRisk.DoesNotExist:
...
class DelegationAdmin(admin.ModelAdmin):
    list_display  = ('name', 'region', 'active_listings', 'avg_sale_price', 'population', 'has_geojson', 'updated_at')
    list_filter   = ('region',)
    ...
    fieldsets = (
        ('Location', {'fields': ('region', 'name')}),
        ('Geospatial', {'fields': ('centroid_lat', 'centroid_lon', 'geojson_polygon')}),
        ('Demographics', {'fields': ('population',)}),
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).annotate(
            _props=Count('property', filter=Q(property__is_active=True)),
            _avg=Avg('property__price', filter=Q(property__is_active=True, property__transaction_type='sale')),
        )
    ...
    @admin.display(description='Avg Sale Price', ordering='_avg')
    def avg_sale_price(self, obj):
        return _money(getattr(obj, '_avg', None))

    @admin.display(description='GeoJSON', boolean=True)
    def has_geojson(self, obj):
        return bool(obj.geojson_polygon)
...
# ─────────────────────────────────────────────────────────────────────────────
# AI MODEL PREDICTIONS  (Feature 3 — stored per-property valuations)
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# PRICE TRENDS  (Feature 5 — time-series forecasting)
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# CLIMATE RISK  (Feature 7 — environmental intelligence layer)
# ─────────────────────────────────────────────────────────────────────────────
