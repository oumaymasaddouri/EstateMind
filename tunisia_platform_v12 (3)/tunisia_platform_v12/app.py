"""
Tunisia Platform v4 — MatchDar + ClimaTN
Fixes: blank screen, climate not loading, images, risk distribution
New: affordability score, market trends, saved searches, property comparison
...
def resolve_cities(city_filter, accept_other):
    if not city_filter or city_filter == "Any":
        return None
    if city_filter in REGION_GROUPS:
        return REGION_GROUPS[city_filter]
    if accept_other == "Yes":
...
def climate_price_adjustment(row):
    category = str(row.get("climate_risk_category", "Moderate"))
    flood = str(row.get("flood_risk", "Medium"))
    heat = str(row.get("heat_risk", "Medium"))
    drought = str(row.get("drought_risk", "Medium"))
    ptype = str(row.get("property_type", "Unknown"))
    city = str(row.get("city", ""))

    category_adj = {"Low": 0.035, "Moderate": 0.0, "High": -0.085, "Very High": -0.16}.get(category, 0.0)
    flood_adj = {"Low": 0.008, "Medium": 0.0, "High": -0.02, "Very High": -0.04}.get(flood, 0.0)
    heat_adj = {"Low": 0.004, "Medium": 0.0, "High": -0.012, "Very High": -0.024}.get(heat, 0.0)
    drought_adj = {"Low": 0.004, "Medium": 0.0, "High": -0.016, "Very High": -0.03}.get(drought, 0.0)

    exposure = {"Apartment": 0.72, "House": 1.0, "Land": 1.15, "Unknown": 0.85}.get(ptype, 0.9)
    coastal = CITIES.get(city, {}).get("coastal", False)
    coastal_adj = -0.012 if coastal and flood in {"High", "Very High"} else (0.006 if coastal and flood == "Low" else 0.0)

    adjustment = ((category_adj + flood_adj + heat_adj * 0.75 + drought_adj * 0.85) * exposure) + coastal_adj
    adjustment = float(np.clip(adjustment, -0.28, 0.08))

    if adjustment >= 0.03:
        label = "Climate premium"
    elif adjustment <= -0.12:
        label = "Strong climate discount"
    elif adjustment < 0:
        label = "Climate discount"
    else:
...
def load_listings():
    ...
    df["city"] = df["city"].astype(str).str.strip().str.title().fillna("Unknown")
    ...
    try:
        ...
        df["images"]        = df["listing_url"].map(lambda u: img_map.get(u, []) if pd.notna(u) else [])
        df["has_images"]    = df["images"].map(lambda x: len(x) > 0 if isinstance(x, (list, tuple)) else False)
        df["primary_image"] = df["images"].map(lambda x: x[0] if isinstance(x, (list, tuple)) and len(x) else "")
    ...
    def _compute_risk(r):
        score = round(
            RISK_NUM.get(r["flood"], 3) * 0.3
            + RISK_NUM.get(r["heat"], 3) * 0.3
            + RISK_NUM.get(r["drought"], 3) * 0.25
            + RISK_NUM.get(r["earthquake"], 1) * 0.15,
            1,
        )
    ...
    df["climate_risk_category"] = climate_info.map(lambda x: x[0][0] if isinstance(x, tuple) and len(x) > 0 and isinstance(x[0], tuple) else "Moderate")
    df["climate_score"]         = climate_info.map(lambda x: x[0][1] if isinstance(x, tuple) and len(x) > 0 and isinstance(x[0], tuple) else 3.9)
    df["flood_risk"]            = climate_info.map(lambda x: x[1].get("flood", "Medium") if isinstance(x, tuple) and len(x) > 1 and isinstance(x[1], dict) else "Medium")
    df["heat_risk"]             = climate_info.map(lambda x: x[1].get("heat", "High") if isinstance(x, tuple) and len(x) > 1 and isinstance(x[1], dict) else "High")
    df["drought_risk"]          = climate_info.map(lambda x: x[1].get("drought", "Medium") if isinstance(x, tuple) and len(x) > 1 and isinstance(x[1], dict) else "Medium")
    ...
    pricing_adjustments = df.apply(climate_price_adjustment, axis=1)
    df["climate_price_adjustment_pct"] = pricing_adjustments.map(lambda x: round(x[0] * 100, 1) if pd.notna(x) and isinstance(x, tuple) and len(x) > 0 else 0.0)
    df["climate_price_label"] = pricing_adjustments.map(lambda x: x[1] if pd.notna(x) and isinstance(x, tuple) and len(x) > 1 else "Climate-neutral")
    df["climate_adjusted_price_tnd"] = (
        df["price_tnd"] * (1 + df["climate_price_adjustment_pct"] / 100.0)
    ).round(0)
    df["pricing_gap_tnd"] = (df["climate_adjusted_price_tnd"] - df["price_tnd"]).round(0)

    city_avg_price_m2 = df[df["surface_m2"] > 0].groupby("city").apply(
        lambda x: (x["price_tnd"] / x["surface_m2"]).mean()
    ).to_dict()
    city_avg_adjusted_price_m2 = df[df["surface_m2"] > 0].groupby("city").apply(
        lambda x: (x["climate_adjusted_price_tnd"] / x["surface_m2"]).mean()
    ).to_dict()
    df["price_per_m2"] = df.apply(
        lambda r: r["price_tnd"] / r["surface_m2"] if r["surface_m2"] > 0 else np.nan, axis=1
    )
    df["climate_adjusted_price_per_m2"] = df.apply(
        lambda r: r["climate_adjusted_price_tnd"] / r["surface_m2"] if r["surface_m2"] > 0 else np.nan, axis=1
    )
    df["city_avg_m2"] = df["city"].map(city_avg_price_m2)
    df["city_avg_adjusted_m2"] = df["city"].map(city_avg_adjusted_price_m2)
    df["affordability_score"] = df.apply(
        lambda r: round(
            np.clip(
                (
                    (100 - (r["price_per_m2"] / max(r["city_avg_m2"], 1)) * 45)
                    + (100 - (r["price_per_m2"] / max(r["city_avg_adjusted_m2"], 1)) * 55)
                ) / 2,
                0,
                100,
            ),
        ...
        if pd.notna(r["price_per_m2"]) and pd.notna(r["city_avg_m2"]) and pd.notna(r["city_avg_adjusted_m2"]) else 50.0,
        axis=1,
    )

    df["listing_id"] = df["record_id"].astype(str)
...
def fetch_weather(city):
    ...
    try:
        ...
        return {
            ...
            "forecast_7d":[{
                "date":dl["time"][i],"temp_max":dl["temperature_2m_max"][i],
                ...
                "uv":(dl.get("uv_index_max") or [0] * 7)[i],
            } for i in range(7)],
        }
    except Exception:
        return simulate_weather(city)
...
def simulate_weather(city):
    ...
    return {
        ...
        "forecast_7d":[{
            ...
            "uv":round(random.uniform(3, 11), 1),
        } for i in range(7)],
    }
...
def hard_filter(user):
    ...
    if amin > 0:
        df = df[(df["surface_m2"] == 0) | (df["surface_m2"] >= amin)]
    if amax > 0:
        df = df[(df["surface_m2"] == 0) | (df["surface_m2"] <= amax)]
    min_r = float(user.get("min_rooms") or 0)
    max_r = float(user.get("max_rooms") or 0)
    if ptype != "Land":
        if min_r > 0:
            df = df[(df["rooms"] == 0) | (df["rooms"] >= min_r)]
        if max_r > 0:
            df = df[(df["rooms"] == 0) | (df["rooms"] <= max_r)]
...
def cbf(df, user, top_n=15):
    ...
    amax = float(user.get("max_area_sqm") or 0)
    uv = {
        "price_tnd": (bmin + bmax) / 2,
        "surface_m2": (amin + amax) / 2 if amax > 0 else amin,
...
def recommend(user, top_n=15):
    ...
    result = result.copy()
    result["final_score"] = (
        result["match_score"].fillna(0) * 0.60
        + result["cbf_similarity"].fillna(0) * 0.40
    ).round(1).clip(0, 100)
...
def api_weather(city):
    ...
    hist = fetch_historical(city)
    rs, rcat = compute_risk_score(city)
    ss, grade = compute_sustainability(city)
    T = w["current"]["temperature"]
    RH = w["current"]["humidity"]
...
def api_compare():
    names = request.args.get(
        "cities",
        "Tunis,Sfax,Sousse,Kairouan,Bizerte,Tozeur,Jendouba,Gabès,Monastir,Mahdia,Ariana,Ben Arous,Nabeul,Le Kef,Gafsa,Médenine",
    ).split(",")
    result = []
    for city in names:
        city = city.strip()
...
def api_price_hint():
    ...
    if ptype and ptype != "Any":
        df = df[df["property_type"] == ptype]
    if df.empty:
        return jsonify({"min":0,"max":0,"mean":0,"count":0,"median":0,
                        "risk_adjusted_min":0,"risk_adjusted_max":0,"risk_adjusted_mean":0,"risk_adjusted_median":0})
    return jsonify({
        "min":int(df.price_tnd.min()),"max":int(df.price_tnd.max()),
        "mean":int(df.price_tnd.mean()),"median":int(df.price_tnd.median()),
        "risk_adjusted_min":int(df.climate_adjusted_price_tnd.min()),
        "risk_adjusted_max":int(df.climate_adjusted_price_tnd.max()),
        "risk_adjusted_mean":int(df.climate_adjusted_price_tnd.mean()),
        "risk_adjusted_median":int(df.climate_adjusted_price_tnd.median()),
        "avg_adjustment_pct":round(float(df["climate_price_adjustment_pct"].mean()), 1),
...
def api_match_stats():
    ...
    return jsonify({
        ...
        "price_stats":{"min":int(PROPS.price_tnd.min()),"max":int(PROPS.price_tnd.max()),
                       ...
                       "risk_adjusted_mean":int(PROPS.climate_adjusted_price_tnd.mean()),
                       "risk_adjusted_median":int(PROPS.climate_adjusted_price_tnd.median()),
                       "avg_adjustment_pct":round(float(PROPS["climate_price_adjustment_pct"].mean()), 1)},
    })
...
def api_regional_heatmap():
    ...
    for city, info in CITIES.items():
        ...
        ss, grade = compute_sustainability(city)
        r = CITY_RISK.get(city, {})
        rt = realtime.get(city, {})
        precip_now = rt.get("precipitation", 0)
        temp_now = rt.get("temperature", 25)
        cv = CITY_CLIMATE_VARS.get(city, {"avg_temp": 20})
        rt_fb = min(precip_now / 15.0, 0.3)
        rt_hb = max(0, (temp_now - cv.get("avg_temp", 20) - 5) / 20)
        ...
        region_map[region].append({
            "city":city,"lat":info["lat"],"lon":info["lon"],"coastal":info["coastal"],
    ...
    for region, cdata in region_map.items():
        ...
        regions_out[region] = {
            "cities":cdata,"n_cities":len(cdata),"avg_historical_risk":avg_hist,
    ...
    return jsonify({
        ...
        "methodology":"Combined risk = historical×0.70 + realtime×0.30",
...
if __name__ == "__main__":
    print(
        f"\n{'=' * 55}\n"
        f"  TUNISIA PLATFORM v4 - Fixed & Improved\n"
        f"  {len(PROPS)} listings | {PROPS['has_images'].sum()} with photos\n"
        f"  {len(CITIES)} climate cities\n"
